...
fun ChatScreen(roomId: String,
    ...
    Column(
        modifier = Modifier.fillMaxSize().padding(16.dp).clickable { focusManager.clearFocus() }
    ) {
        LazyColumn (
...
private fun formatTimeStamp(timeStamp: Long): String {
    ...
    return when {
        ...
        isSameDay(messageDateTime.minusDays(1), now) -> "yesterday ${formatTime(messageDateTime)}"
...
fun ChatMessageItem(message: Message){
    Column(modifier = Modifier.fillMaxWidth()
        ...
        else Alignment.Start // the message will start from the right if is sent
        //by current user
        ) {
        Box (
            modifier = Modifier.background(
                color = if(message.isSentByCurrentUser) colorResource(id = R.color.purple_700)
                else Color.DarkGray,
